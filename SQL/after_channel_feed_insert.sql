DELIMITER //
DROP TRIGGER IF EXISTS `teltonika`.`after_channel_feed_insert` //
...
BEGIN
    ...
    -- Si temperatura > v_temp_inicio_faena y no hay faena activa -> crear nueva faena
    ...
    IF v_faena_activa_id IS NOT NULL THEN
        ...
        -- Si temperatura <= v_temp_fin_faena, verificar si debemos cerrar la faena
        IF NEW.field7 <= v_temp_fin_faena THEN
            ...
            IF v_tiempo_transcurrido >= v_tiempo_fin_faena THEN
                ...
                -- Generar resumen de la faena
                CALL generar_resumen_faena(v_faena_activa_id);
            END IF;
        END IF;
    END IF;
END//
