...
const CustomTooltip = (context) => {
  ...
  const tooltipMonth = moment().month(parseInt(dataPoints[0].label) - 1).format("MMMM");
...
const ConsumoTotalAnual = () => {
  ...
  const chartData = {
    labels: Array.from({ length: 12 }, (_, i) => (i + 1).toString().padStart(2, '0')),
    datasets: [
      {
        type: "bar",
        label: "Consumo Mensual (kWh)",
        data: Array.from({ length: 12 }, (_, i) => {
          const monthData = data.find(item => parseInt(moment(item.fecha_local).format('M')) === i + 1);
          return monthData ? parseFloat(monthData.energia_activa_total) : 0;
        }),
      ...
      },
      {
        ...
        data: Array.from({ length: 12 }, (_, i) => {
          const monthData = data.find(item => parseInt(moment(item.fecha_local).format('M')) === i + 1);
          return monthData ? parseFloat(monthData.costo_total) : 0;
        }),
    ...
    scales: {
      x: {
        ...
        ticks: {
          ...
          callback: (value) => moment().month(parseInt(value) - 1).format("MMM"),
        },
        title: {
          display: true,
          text: "Mes",
