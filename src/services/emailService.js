...
class EmailService {
    ...
    async sendTemperatureRangeAlertsEmail(
        ...
        if (!forceOutsideWorkingHours && !this.isWithinWorkingHours(currentTime)) {
            ...
            console.log(
                `Email Service - Fuera de horario laboral (${workingHoursDesc}). No se envían alertas de temperatura.`
            );
            return false;
        }
    ...
    async sendIntrusionAlertEmail(
        ...
        if (!sector) {
            console.error(
                "Email Service - Error: Sector no especificado para alerta de intrusión."
            );
            return false;
        }

        // Verificar y usar los destinatarios proporcionados directamente
        const emailRecipients = recipients || this.defaultRecipients;

        if (!emailRecipients || emailRecipients.length === 0) {
            console.error(
                "Email Service - No hay destinatarios configurados para la alerta de intrusión."
            );
            return false;
        }
        ...
        const alertId = `ALERT-${Date.now()}-${sector
            .replace(/\s/g, "")
            .substring(0, 8)
            .toUpperCase()}`;
...
${companyName} - ${appName}`,
      ...
      `,
        ...
        try {
            await sgMail.send(message);
            console.log(
                `Correo de alerta de intrusión enviado a: ${emailRecipients.join(", ")}`
            );
    ...
    async sendGenericEmail(
        ...
        if (!subject || !content) {
            console.error(
                "Email Service - Error: Asunto o contenido no proporcionados para correo genérico."
            );
            return false;
        }
        ...
        if (!recipientList || recipientList.length === 0) {
            console.error(
                "Email Service - No hay destinatarios configurados para el correo genérico."
            );
            return false;
        }
        ...
        if (isHtml) {
            ...
            if (
                ...
                !content.includes("margin-top: 30px; padding-top: 15px; border-top:")
            ) {
                const signature = `
          <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #e1e1e1; font-size: 12px; color: #777;">
            <p>Este es un mensaje automático del sistema de ${companyName}. Por favor no responda a este correo.</p>
        ...
        `;
            ...
            if (
                !content.includes(companyName) &&
                !content.includes("mensaje automático")
            ) {
        ...
        try {
            await sgMail.send(message);
            console.log(
                `Email Service - Correo "${subject}" enviado a: ${recipientList.join(
                    ", "
                )}`
            );
