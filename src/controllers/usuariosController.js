...
class usuariosController {
  async handleLogin(req, res, next) {
    ...
    try {
      ...
      const jwtSecret =
        process.env.JWT_SECRET || jwtConfig.getJwtConfig().secret;
      const jwtIssuer =
        process.env.JWT_ISSUER || jwtConfig.getJwtConfig().issuer;
      const jwtExpiresIn =
        process.env.JWT_EXPIRES_IN || jwtConfig.getJwtConfig().expiresIn;
  ...
  async requestPasswordReset(req, res) {
    ...
    try {
      ...
      // Reemplazar todo este bloque con:
      const emailService = require("../services/emailService");
      await emailService.sendPasswordResetEmail(
        resetData.email,
        resetToken,
        `${req.protocol}://${req.get("host")}${resetUrl}`
      );
