  ...
  try {
    ...
    if (response.data.includes("<response>OK</response>")) {
      ...
      if (response.data.includes("<error>")) {
        ...
        throw new Error(
          `Error SMS - Código: ${
            errorCode ? errorCode[1] : "Desconocido"
          }, Mensaje: ${errorMessage ? errorMessage[1] : "Sin mensaje"}`
        );
      }
...
// Reemplazar el método enviarCorreoIncidencia en control_incidencias.js (líneas 155-189)
async function enviarCorreoIncidencia(asunto, mensaje) {
  const emailService = require("../services/emailService");

  // Extraer información del mensaje para formato estructurado
  // (Esto es un ejemplo aproximado, ajustar según el formato real de los mensajes)
  const sectorMatch = mensaje.match(/sector:\s*([^\n]+)/);
  const sector = sectorMatch ? sectorMatch[1].trim() : "No especificado";

  const intruderMatch = mensaje.match(/Intruso\/Individuo:\s*([^\n]+)/);
  const intruderInfo = intruderMatch
    ? intruderMatch[1].trim()
    : "No identificado";

  return await emailService.sendIntrusionAlertEmail(
    sector,
    intruderInfo,
    new Date(),
    sgMailConfig.email_contacto.destinatarios,
    true
  );
}
...
async function procesarPosibleIncidencia(
  ...
  try {
    ...
    if (!esBlindSpot) {
      console.log(
        `[DEBUG] El dispositivo ${device_name} no es blind spot - Terminando`
      );
      return;
    }
    ...
    if (hayBeacons) {
      ...
      const beaconList =
        typeof ble_beacons === "string" ? JSON.parse(ble_beacons) : ble_beacons;

      // Procesar cada beacon de manera independiente
      for (const beacon of beaconList) {
        console.log(
          `[DEBUG] Verificando beacon: ${beacon.id}, RSSI: ${beacon.rssi}`
        );
        ...
        if (beaconEsBlindSpot && beacon.rssi > RSSI_THRESHOLD) {
          ...
          if (!beaconCooldowns.has(cooldownKey)) {
            ...
            console.log(
              `[DEBUG] Cooldown activo para ${cooldownKey} - Skip inserción y SMS`
            );
          }
        }
      }
...
async function procesarBeacon(device_name, beacon) {
  try {
    ...
    console.error(
      `[${new Date().toISOString()}] Error en procesarBeacon:`,
      error
    );
  }
}
...
async function enviarSMSIncidencia(dispositivo, detector_id) {
  try {
    const puedeEnviarSMS = await verificarTiempoUltimoSMS(
      dispositivo,
      detector_id
    );
    if (!puedeEnviarSMS) {
      console.log(
        `No se puede enviar SMS aún para ${dispositivo} - Debe esperar el intervalo`
      );
      return;
    }
    ...
    try {
      ...
      await sendSMS(
        smsConfig.sms_destinatarios.activacion.numero,
      ...
      await sendSMS(
        smsConfig.sms_destinatarios.confirmacion.numero,
...
async function comprobarBlindSpot(inputToCheck, caso) {
  ...
  try {
    ...
    console.log(
      `Resultado de comprobarBlindSpot para ${inputToCheck}: ${esBlindSpot}`
    );
    return esBlindSpot;
  } finally {
    connection.release();
