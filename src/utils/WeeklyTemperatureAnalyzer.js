...
class WeeklyTemperatureAnalyzer {
    constructor() {
     ...
     calculateTemperatureDistribution(period) {
        ...
        dayNames.forEach(day => {
            distribution.set(day, ranges.map(range => ({
                range: range.label,
                count: 0,
                percentage: 0
            })));
        ...
        for (let dayIndex = 1; dayIndex <= 7; dayIndex++) {
            ...
            this.records[period].filter(record => record.datetime.isoWeekday() === dayIndex).forEach(record => {
                const temp = record.temperature;
                const dayData = distribution.get(dayName);
    
                const rangeIndex = ranges.findIndex(range => temp >= range.min && temp <= range.max);
                if (rangeIndex >= 0) {
        ...
        distribution.forEach((dayData, day) => {
            const totalReadings = dayData.reduce((sum, range) => sum + range.count, 0);
            let totalPercentage = 0;
    
            dayData.forEach(range => {
                range.percentage = totalReadings > 0 ? Math.round((range.count / totalReadings) * 100) : 0;
    ...
    async analyzeData(currentData, previousData, cameraName, dataDate) {
        ...
        const selectedDate = moment(dataDate).tz('America/Santiago');
         ...
         const prevStartDate = selectedDate.clone().subtract(13, 'days').startOf('day');
         const prevEndDate = selectedDate.clone().subtract(7, 'days').endOf('day');

        console.log('analyzeData - Calculated Date Ranges:');
        console.log('  Current Week Start:', startDate.format('YYYY-MM-DD HH:mm:ss'));
        console.log('  Current Week End:', endDate.format('YYYY-MM-DD HH:mm:ss'));
        console.log('  Previous Week Start:', prevStartDate.format('YYYY-MM-DD HH:mm:ss'));
        console.log('  Previous Week End:', prevEndDate.format('YYYY-MM-DD HH:mm:ss'));
        ...
        .filter(record => {
         ...
         this.records.previous = previousData
            ...
            .filter(record => {
                return record.datetime.isBetween(prevStartDate, prevEndDate, null, '[]');
            });
        ...
        console.log('Sample of current records:', this.records.current.slice(0, 5).map(r => ({
        ...
        const sundayRecords = this.records.current.filter(r => r.datetime.isoWeekday() === 7);
        ...
        console.log('Date range verification:', {
            current: {
                firstRecord: this.records.current.length > 0 ? 
                    this.records.current[0].datetime.format('YYYY-MM-DD HH:mm:ss') : 'No records',
                lastRecord: this.records.current.length > 0 ? 
                    this.records.current[this.records.current.length - 1].datetime.format('YYYY-MM-DD HH:mm:ss') : 'No records'
            },
            previous: {
                firstRecord: this.records.previous.length > 0 ? 
                    this.records.previous[0].datetime.format('YYYY-MM-DD HH:mm:ss') : 'No records',
                lastRecord: this.records.previous.length > 0 ? 
                    this.records.previous[this.records.previous.length - 1].datetime.format('YYYY-MM-DD HH:mm:ss') : 'No records'
            }
        });
    }
    ...
    calculateStats(period) {
        ...
        this.stats[period].promedio = temperatures.reduce((a, b) => a + b, 0) / temperatures.length;
        this.stats[period].desv_std = Math.sqrt(
            temperatures.reduce((a, b) => a + Math.pow(b - this.stats[period].promedio, 2), 0) /
            temperatures.length
        );
    ...
    calculateDailyPatterns(period) {
        ...
        for (let dayIndex = 1; dayIndex <= 7; dayIndex++) {
            ...
            console.log(`${period} - Day ${dayIndex} (${dayNames[dayIndex-1]}) records:`, dayData.length);
    ...
    async generateTemperatureChart(period, dataDate) {
     ...
     drawPageHeader(doc, cameraName, dataDate) {
        try {
            doc.image('./src/assets/images/thenextlogo.png', 70, 30, { width: 100, height: 75 });
             doc.image('./src/assets/images/storagelogo.png', 600, 30, { width: 100, height: 75 }, { align: 'right' });
          } catch (error) {
        ...
        doc.moveDown(2)
            ...
            .fontSize(14)
            .text(`Correspondiente a: ${cameraName}`, { align: 'center', font: 'Helvetica-Bold' })
            .fontSize(12)
            .text(`Fecha de los Datos Analizados: ${moment(dataDate).format('DD-MM-YYYY')} , Semana ${this.stats.current.weekNumber}`, { align: 'center' })
            .moveDown(0.5)
            .text('Analiza Inter-Semanas', { align: 'center' })
            .text(`Fecha de Análisis: ${moment().format('DD-MM-YYYY HH:mm')}`, {align: 'center'})
            .moveDown(2)
    }
    async addChartSection(doc, period, dataDate, x, y) {
        // Determinar título y número de semana según el período
        const weekNumber = period === 'current' ? this.stats.current.weekNumber : this.stats.previous.weekNumber;
        const title = `Valores Detallados por Día Período ${period === 'current' ? 'Actual' : 'Anterior'} - Semana ${weekNumber}`;
        ...
        const dailyData = Array.from(this.dailyAverages[period].entries())
            .sort((a, b) => a[0] - b[0])
            .map(([dayIndex, data]) => ({
                day: ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'][dayIndex - 1],
    ...
    async generatePDFContent(doc, cameraName, dataDate) {
        ...
        this.drawPageHeader(doc, cameraName || 'Sin nombre', dataDate || 'Sin fecha');
        ...
        doc.fillColor('#f0f0f0')
            .rect(startX, yPos,  // Cambiar de tableConfig.margin.left a startX
                tableConfig.columns.reduce((sum, col) => sum + col.width, 0),
        ...
        rows.forEach((rowData, index) => {
            ...
            rowData.forEach((text, i) => {
                ...
                doc.font(isCurrentDateColumn ? 'Helvetica-Bold' : 'Helvetica')
                    .text(text,
                        xPos,
        ...
        doc.fontSize(12)
           .text(`Valores Detallados por Día Período Actual - Semana ${this.stats.current.weekNumber}`, 
                 doc.page.margins.left, 
                 startY, 
                 { width: chartWidth, underline: true });
    
        const chartBufferCurrent = await this.generateTemperatureChart('current', dataDate);
        if (chartBufferCurrent) {
            doc.image(chartBufferCurrent, 
                     doc.page.margins.left, 
                     startY + 30, 
        ...
        doc.fontSize(12)
           .text(`Valores Detallados por Día Período Anterior - Semana ${this.stats.previous.weekNumber}`, 
                 doc.page.margins.left + chartWidth, 
                 startY, 
                 { width: chartWidth, underline: true });
    
        const chartBufferPrevious = await this.generateTemperatureChart('previous', dataDate);
        if (chartBufferPrevious) {
            doc.image(chartBufferPrevious, 
                     doc.page.margins.left + chartWidth, 
                     startY + 30, 
                     { fit: [chartWidth - 20, 180] });
        }
        ...
        await this.addDetailedValuesTable(doc, 'previous', dataDate, 
                                        doc.page.margins.left + chartWidth, 
                                        startY + 220);
    ...
    async generatePDF(cameraName, dataDate) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                const currentDistChart = await this.generateDistributionChart('current');
                const previousDistChart = await this.generateDistributionChart('previous');
           ...
           if (currentDistChart) {
               doc.image(currentDistChart, 
                        doc.page.margins.left, 
                        startY + 10,
                        { fit: [chartWidth - 20, 400] });
           }
           if (previousDistChart) {
               doc.image(previousDistChart,
                        doc.page.margins.left + chartWidth,
                        startY + 10,
                        { fit: [chartWidth - 20, 400] });
           }
        ...
        //  Add the data tables
         const currentTableY = startY + 270;
          await this.addDistributionDataTable(doc, 'current', doc.page.margins.left, currentTableY);
          
          const previousTableY = startY + 270;
        await this.addDistributionDataTable(doc, 'previous', doc.page.margins.left + chartWidth, previousTableY);
...
async addDistributionDataTable(doc, period, tableX, tableY) {
    ...
    let headerX = startX;
    tableConfig.columns.forEach(column => {
        doc.text(column.header, headerX, yPos + (tableConfig.headerHeight - tableConfig.fontSize) / 2,
            { width: column.width, align: 'center' });
        headerX += column.width;
    });
    ...
    ranges.forEach((rangeLabel, rangeIndex) => {
        ...
        doc.text(rangeLabel, rowX, yPos + (tableConfig.rowHeight - tableConfig.fontSize) / 2, { width: tableConfig.columns[0].width, align: 'center' });
        ...
        dayNames.forEach(dayName => {
            ...
            doc.text(value.toString(), rowX, yPos + (tableConfig.rowHeight - tableConfig.fontSize) / 2, { width: tableConfig.columns[1].width, align: 'center' });
    ...
    async addDetailedValuesTable(doc, period, dataDate, tableX, tableY) {
        ...
        Array.from(this.dailyAverages[period].keys())
            ...
            .forEach((dayIndex, index) => {
                if (yPos + tableConfig.rowHeight > doc.page.height - doc.page.margins.bottom) {
                   doc.addPage();
