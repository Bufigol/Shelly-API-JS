app.get('/api/temperature-power-analysis', async (req, res) => {
    ...
    try {
        ...
        const [results] = await pool.query(query, [date, ubicacion, date, channelId]);
  ...
  app.get('/api/temperature-power-locations', async (req, res) => {
    try {
        const query = `
            SELECT DISTINCT 
